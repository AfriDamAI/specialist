...
export default function ConsultationsHubPage() {
  ...
  useEffect(() => {
    ...
    async function fetchSessions() {
      try {
        // Rule #3: Precision Path - Backend uses singular 'consultation'
        const response = await fetch(`${API_URL}/consultation`);
        const data = await response.json();
        
        if (data.succeeded && data.data) {
          // Rule #5: Sorting cases by newest first for priority triage
  ...
  return (
    <DashboardLayout>
      <div className="max-w-7xl mx-auto space-y-6 pb-24 text-left animate-in fade-in duration-700">
        ...
        <div className="relative min-h-[500px]">
          ...
          {!isVerified && (
            <div className="absolute inset-0 z-[60] backdrop-blur-xl bg-white/20 dark:bg-gray-950/20 flex items-center justify-center rounded-[3rem] border-2 border-dashed border-gray-100 dark:border-gray-800">
              <div className="bg-white dark:bg-gray-900 p-10 rounded-[3rem] shadow-2xl text-center max-w-sm border border-gray-100 dark:border-gray-800 animate-in zoom-in duration-300">
                ...
                <h3 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter italic">Queue Locked</h3>
          ...
          <div className={`space-y-4 transition-all duration-500 ${!isVerified ? 'opacity-5 pointer-events-none select-none' : 'opacity-100'}`}>
            
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-20">
                <ArrowPathIcon className="w-8 h-8 text-[#FF7A59] animate-spin mb-4" />
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest text-center italic">Hydrating Clinical Queue...</p>
              </div>
            ) : isVerified && sessions.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center bg-gray-50/50 dark:bg-gray-900/50 rounded-[3rem] border border-gray-100 dark:border-gray-800">
                ...
                <h3 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter italic">No Active Cases</h3>
                <p className="text-[10px] text-gray-400 font-black mt-2 uppercase tracking-[0.2em] italic">
              ...
              sessions.map((session) => (
                <Link 
                  href={`/consultation/${session.id}`} // Rule #3: Consistent singular path
                  key={session.id} 
                  className="block bg-white dark:bg-gray-800 p-6 md:p-8 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 hover:border-[#FF7A59] hover:shadow-xl transition-all group animate-in slide-in-from-bottom-2 duration-300"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-5">
                      <div className="w-14 h-14 bg-gray-50 dark:bg-gray-900 rounded-2xl flex items-center justify-center text-xl font-black text-[#FF7A59] uppercase italic shadow-inner">
                        {(session.name || 'P').charAt(0)}
                      </div>
                      <div>
                        <h3 className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tighter leading-none group-hover:text-[#FF7A59] transition-colors italic">
                          {session.name || 'Emergency Triage'}
                        </h3>
                        <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 mt-2 uppercase tracking-widest line-clamp-1 italic">
                    ...
                    <div className="flex items-center gap-6">
                      <div className="hidden md:flex flex-col items-end">
                        <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest italic">Received</span>
                        <span className="text-[11px] font-black text-gray-900 dark:text-white uppercase italic">
                          {new Date(session.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </div>
                      <div className="bg-gray-50 dark:bg-gray-900 p-3 rounded-xl group-hover:bg-[#FF7A59] group-hover:text-white transition-all active:scale-95 shadow-sm">
