  ...
  return (
    <DashboardLayout>
      <div className="max-w-6xl mx-auto p-4 md:p-8">
        <div className="mb-8">
          <h1 className="text-3xl font-black text-(--dashboard-text) uppercase italic tracking-tighter">
            Specialist <span className="text-[#FF7A59]">Wallet</span>
          </h1>
          <p className="text-(--dashboard-text-muted) font-medium">Manage your clinical earnings and withdrawals.</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="dashboard-card rounded-4xl md:rounded-[2.5rem] p-6 sm:p-8 relative overflow-hidden group">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="dashboard-card rounded-4xl p-5 sm:p-6">
                  ...
                  <p className="text-lg font-black text-(--dashboard-text) italic">{formatCurrency(wallet?.totalIn || 0)}</p>
                </div>
                <div className="dashboard-card rounded-4xl p-5 sm:p-6">
                  ...
                  <p className="text-lg font-black text-(--dashboard-text) italic">{formatCurrency(wallet?.totalOut || 0)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-2">
              <div className="dashboard-card rounded-4xl md:rounded-[2.5rem] overflow-hidden">
                <div className="p-6 md:p-8 border-b border-(--dashboard-border) flex items-center justify-between bg-(--dashboard-surface-soft)">
                  <h3 className="text-lg font-black text-(--dashboard-text) uppercase italic tracking-tighter">Transaction History</h3>
                ...
                <div className="divide-y divide-(--dashboard-border)">
                  {transactions.length === 0 ? (
                    ...
                    transactions.map((tx) => (
                      <div key={tx.id} className="p-5 md:p-6 hover:bg-(--dashboard-surface-soft) transition-colors flex items-center justify-between group">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-sm font-black text-(--dashboard-text) uppercase italic truncate max-w-50 sm:max-w-md">
                              {tx.description}
                            </p>
      ...
      {isWithdrawModalOpen && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="dashboard-card rounded-4xl md:rounded-[3rem] w-full max-w-md p-6 sm:p-8 animate-in zoom-in duration-300">
            <div className="text-center mb-8">
              <div className="w-20 h-20 bg-[#FF7A59]/10 rounded-4xl flex items-center justify-center mx-auto mb-6">
                <CurrencyDollarIcon className="w-10 h-10 text-[#FF7A59]" />
              </div>
              <h3 className="text-2xl font-black text-(--dashboard-text) uppercase italic tracking-tighter mb-2">
                Withdraw <span className="text-[#FF7A59]">Earnings</span>
              </h3>
              <p className="text-sm text-(--dashboard-text-muted) font-medium px-4">
            ...
            <form onSubmit={handleWithdrawalRequest} className="space-y-6">
              <div>
                ...
                <div className="mt-3 flex items-center justify-between px-2">
                    <p className="text-[10px] font-black text-(--dashboard-text-muted) uppercase tracking-widest italic">
                    Available: <span className="text-(--dashboard-text)">{formatCurrency(wallet?.balance || 0)}</span>
                  </p>
                  <button 
                    type="button"
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 dashboard-icon-tile hover:bg-(--dashboard-surface-soft) text-gray-600 dark:text-gray-300 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all"
                >
                  Cancel
                </button>
                <button
